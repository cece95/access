...
def get_prediction_on_headlines(simplifier, phase):
    source_filepath = get_data_filepath('headlines', phase, 'complex')
    pred_filepath = get_temp_filepath()
    with mute():
        simplifier(source_filepath, pred_filepath)
    return pred_filepath
...
def evaluate_simplifier_on_headlines(simplifier, phase):
